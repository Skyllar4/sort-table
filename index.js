(() => {
    ...
    const data = [
      ...
      ]; // я не стал делить приложение на модули, так как не имею сервера для этого, поэтому данные разместил в этом фалйе  
    

    function createTable(objArr) { // Функция, генерирует базовую структуру таблицы

        if (document.querySelector('table')) {
          document.querySelector('.sort-container').remove();
          document.querySelector('table').remove(); // Очистка прошлой таблицы для генерации новой
        }
        ...
        const titleTableData = objArr[0]; // Получаем первый объект, для установления заголовков таблицы
        const trColumns = Object.keys(titleTableData); // получаем ключи объекта
        trColumns.shift(); // обрезаем id
        createSortInput(trColumns);
        const titleArr = [] // Для хранения заголовков, чтобы потом передать этот массив в createSortInput

        for (key in titleTableData) {
            if (key === 'id') {
                continue;
            } else if (titleTableData[key] === undefined || titleTableData[key] === null) { // небольшой предохранитель от ошибок
                createTitleCell('');
            } else if (typeof titleTableData[key] !== 'string') { // обработка входных данныХ, если есть значения типа name

                const treatmentArr = arrOrObj(titleTableData[key], 'keys');

                for (index of treatmentArr) {
                    if (index === undefined) {
                        break;
                    }
                    titleArr.push(index);
                    createTitleCell(index);
                }
                continue;
        ...
        mainContainer.prepend(createSortInput(titleArr)); 

                /**
         * Создает заголовок ячейки, чтобы не дублировать код в цикле выше.
         * Функция создется внутри функции, чтобы использовать замыкания
        ...
        function createTitleCell(value) { 
            ...
            const entTitle = document.createElement('th');
            const checkBox = document.createElement('input');
            entTitle.textContent = value;
            entTitle.classList.add(value);
            ...
            entTitle.append(checkBox);
            tr.append(entTitle);
    
        }
        createBodyTable(objArr);

        }; createTable(data);

        function createBodyTable(objArr) { // функция, которая рендерит таблицу с данными

            const table = document.querySelector('table'); // приложение предусматривет 1 таблицу, так что считаю это решение обоснованным

            for (i of objArr) {

                const strGroup = document.createElement('tr'); // соглавсно какой-то статье в документации, не нужно создавать tbody через js, так как браузер сделает это за нас, поэтому я не делаю tbody
                 strGroup.setAttribute('id', i.id);

                    for (key in i) {   // итерация по значениям ключа очередного объекта

                        if (key === 'id') {
                            continue;
                        } else if (i[key] === undefined || i[key] === null) { // небольшой предохранитель от ошибок
                            strGroup.append(createCell(''));
                        } else if (typeof i[key] !== 'string') { // обработка входных данныХ, если есть значения типа name

                            const treatmentArr = arrOrObj(i[key], 'value');

                            for (index of treatmentArr) {
                                if (index === undefined) {
                                    break;
                                }
                                if (Object.entries(i[key])[0].includes(index)) {
                                  strGroup.append(createCell(index, 'firstName'));
                                } else if (Object.entries(i[key])[1].includes(index)) {
                                  strGroup.append(createCell(index, 'lastName'));
                                } 
                            }
                            continue;
                        } else if (key === 'about') {
                            strGroup.append(createCell(i[key], key, true));
                            continue;
                        }
                        strGroup.append(createCell(i[key], key));
                    } // не использую Object.values, потому, что все равно придется запускать цикл

                table.append(strGroup);

            }
        };

        /**
         * Создает ячейку с нужным значением.
         * @param {string} value Значение ячейки.
         * @param {string} objKey Значения ключа, для назначения класса, по ним потом будут скрываться колонки
         * @return {HTMLAttributes}, функция возвращает HTML элемент.
         */

        function createCell(value, objKey, bigContent = false) {
            
            const cell = document.createElement('td');
            cell.classList.add('cell');
            if (bigContent) {
              cell.classList.add('bigCell');
            }
            if (value === 'red' || value === 'blue' || value === 'brown' || value === 'green') {
              cell.classList.add(value);
              cell.classList.add(objKey);
              cell.addEventListener('dblclick', (el) => createChangeFrom(el));
              return cell;
            }
         ...
         * @param {HTMLElement} e Элемент, вызвавший событие.
         * @return {void} Функция ничего не возвращает
         */

        function createChangeFrom(e) {

          createChangeFormClosed(); // зыкрываем форму, на случай, если открыта другая
          ...
          changeForm.append(accBtn);
          changeForm.append(decBtn);
          formContainer.append(changeForm);
          document.body.append(formContainer);

          changeInput.setAttribute('value', e.target.textContent);
          changeInput.setAttribute('id', 'updInput');
          changeInput.classList.add('d-block');

          accBtn.textContent = 'Сохранить';
          decBtn.textContent = 'Отмена';
          accBtn.classList.add('accBtn');
          accBtn.addEventListener('click', (event) => {
              event.preventDefault();
              updateTableCell(e, document.getElementById('updInput').value);
          });
          decBtn.classList.add('decBtn');
          decBtn.addEventListener('click', (event) => {
            event.preventDefault();
            createChangeFormClosed();
          });

          formContainer.classList.add('change-container');
          formContainer.style.left = e.pageX + 64 + 'px';
          formContainer.style.top = e.pageY - 23 + 'px'; // создаем форму, в зависимости от координат с небольшими поправками, чтобы див всегда создавался в соседнем контейнере

        }

        function createChangeFormClosed() { // Boolean, если нужно закрыть форму полностью, то false, если форма пересоздается, то true
          
          if (document.querySelector('.change-container')) {
              document.querySelector('.change-container').remove();
          }

        }

        function updateTableCell(el, inputValue) { // Функция, которая ищет и редактирует нужное поле в data и перерендеривает приложение el - Ячейка, по которой был произведен клик, для последующего вычисления родителя tr и получения id

          const uptEl = data.find(elem => elem.id === el.target.parentNode.id); // сначала ищем нужный объект
          const keysAndValues = Object.entries(uptEl)

          for (index of keysAndValues) {

            if (index.includes(el.target.textContent)) {

              data[data.indexOf(uptEl)][index[0]] = inputValue.toString();
              createTable(data);

              return;

            } else if (index.includes('blue') || index.includes('red') || index.includes('green') || index.includes('brown')) {
              ...
              return;
            ...
            } else if (typeof index[1] === "object" && !Array.isArray(index[1]) && Object.entries(index[1])[0].includes(el.target.textContent) || Object.entries(index[1])[1].includes(el.target.textContent)) {

                if (Object.values(index[1]).includes(el.target.textContent)) {

                  Object.entries(index[1]).forEach((i) => {

                    if (i.includes(el.target.textContent)) {

                      data[data.indexOf(uptEl)][index[0]][i[0]] = inputValue.toString();
                      createTable(data);
                ...
                } // выше какая то грязища происходит, но это работает
        ...
        function arrOrObj(value, keyOrValue) { // keyOrValue, указывается аргумент, что мы хотим получить от значиения на входе, допустим при создании заголовокв, нужно достать ключи, а при колонках значение
            if (value === 'string') { // функция для обработки типов, чтобы можно было добавить в базу массив или еще один объект и ничего не сломалось, проверяет тип и возвращает массив данных
                return false;
            } else if (Array.isArray(value)) {
                return value;
            } else if (typeof value === 'object' && keyOrValue === 'value') {
                return Object.values(value);
            } else if (typeof value === 'object' && keyOrValue === 'keys') {
        ...
        function createSortInput(arr) { // Создание кнопок для сортировки
          ...
          for (i of arr) {
            ...
            const btnGroupSort = document.createElement('div');
            btnGroupSort.setAttribute('id', i); // id для родителя, так как у нас 2 кнопки
            const ascButton = document.createElement('button');
            const descBtn = document.createElement('button');

            ascButton.addEventListener('click', () => sortValues(data, ascButton.parentNode.id, 'asc'));
            descBtn.addEventListener('click', () => sortValues(data, descBtn.parentNode.id, 'desc'));

            btnGroupSort.append(ascButton);
            ascButton.textContent = 'Сортировка по возрастанию';
            btnGroupSort.append(descBtn);
            descBtn.textContent = 'Сортировка по убыванию';
            ascButton.classList.add('sort-btn');
            descBtn.classList.add('sort-btn');
            sortContainer.append(btnGroupSort);

          }
          
          return sortContainer;

        } // В ТЗ написано сортировка, поэтому у меня именно сортировка, не фильтрация

        /**
         * Сортирует таблицу по убыванию или по возрастанию.
         * @param {Array} arr Исходный массив c объектами для сортировки.
         * @param {string} id, значение поля в объекте по которому будет происзовдиться сортировка.
         * @param {string} method - Метод сортировки по возрастанию или по убыванию DESC-ASC
         * @return {void || false} Функция перередеривает талицу или возращает false, если метод был передан неккоректно
         */

      function sortValues(arr, id, method) { // сортировка массива и ререндер таблицы

        let objValue; // Переменная для записи ключа, содержащего объект

        if (!Object.keys(arr[0]).includes(id)) { // Проверка, есть ли в объекте данный ключ для сортировки, если нет, то ищем, в каком элементе есть данный ключ
            for (i in arr[0]) { // ищем объект, в котором есть firstName или lastName
              if (arr[0][i].hasOwnProperty(id)) {
                  objValue = i;
                  break;
              }
            }
            ...
            if (method === 'asc') {
              ...
              const arrSorted = arr.sort((a, b) => a[objValue][id] < b[objValue][id] ? 1 : -1);
              createTable(arrSorted);
        ...
        if (method === 'asc') {
          ...
          const arrSorted = arr.sort((a, b) => a[id] > b[id] ? 1 : -1);
          createTable(arrSorted);
         ...
         // Возможно здесь вы хотели бы увидеть алгоритм сортировки хотя бы пузырьком, но я использую метод, я полагаю, что он будет быстрее
